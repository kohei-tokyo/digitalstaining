    ...
    def test(self):
        ...
        for self.test_id in test_list:
            ...
            if self.test_id != "final":
                self.G.load_state_dict(torch.load(f"best_model_G_stain_{self.test_id}_{self.name}.pth"))
                self.D.load_state_dict(torch.load(f"best_model_D_stain_{self.test_id}_{self.name}.pth"))
    ...
    def save_results(self, total_metrics_dict, mode):
        ...
        if mode == "val":
            ...
            if mean_mse < self.min_val_loss_mse:
                ...
                torch.save(self.G.state_dict(), f"best_model_G_stain_mse_{self.name}.pth")
                torch.save(self.D.state_dict(), f"best_model_D_stain_mse_{self.name}.pth")
            if mean_ssim < self.min_val_loss_ssim:
                ...
                torch.save(self.G.state_dict(), f"best_model_G_stain_ssim_{self.name}.pth")
                torch.save(self.D.state_dict(), f"best_model_D_stain_ssim_{self.name}.pth")
            if mean_lpips < self.min_val_loss_lpips:
                ...
                torch.save(self.G.state_dict(), f"best_model_G_stain_lpips_{self.name}.pth")
                torch.save(self.D.state_dict(), f"best_model_D_stain_lpips_{self.name}.pth")
